...
export const BUILT_IN_COMPONENTS: CustomComponentDef[] = [
  ...
  <section style="display: table-cell; vertical-align: middle; width: 64px;">
    <section style="width: 56px; height: 56px; border-radius: 50%; background-image: url('{{avatar}}'); background-size: cover; background-position: center 20%;"></section>
  </section>
...
{{#each tags}}<span style="display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 13px; font-weight: 500; background: {{color}}1a; color: {{color}}; border: 1px solid {{color}}40;">{{item}}</span>{{/each}}
  ...
  },
  {
    ...
    props: [
      ...
      { name: `avatarWidth`, description: `头像宽度：small | large | full 或数字 px`, default: `small` },
    ...
    template: ``,
    example: `<AboutAuthor name="陈恒" avatar="https://raw.githubusercontent.com/BluerAngala/cc-md-editor/main/apps/web/public/images/chenheng.jpg" title="广东岭南律师事务所·专职律师" bio="您好，我是陈恒，主要业务是政府法律顾问和公司法务，擅长处理民商事纠纷、政府行政案件、企业合规事务等。业余积极探索法律AI科技实践，创建「法律AI 科技实践基地」知识库、开发「陈恒律师工具箱」小程序等。" style="lawyer" />`,
    examples: [
      { label: `蓝底`, example: `<AboutAuthor name="张三" avatar="https://avatars.githubusercontent.com/u/21008209?v=4" title="全栈工程师 / 开源爱好者" bio="热爱技术，专注于 Web 开发与开源社区。" style="blue" avatarWidth="large" avatarShape="round" />` },
      ...
      { label: `律师`, example: `<AboutAuthor name="陈恒" avatar="https://raw.githubusercontent.com/BluerAngala/cc-md-editor/main/apps/web/public/images/chenheng.jpg" title="广东岭南律师事务所专职律师 / 产业投资专业委员会委员" bio="法学专业，爱好科技，略懂 AI 和自动化，业余折腾了一些法律科技有关的项目。如有志同道合的朋友，欢迎交流探讨。" style="lawyer" />` },
      { label: `企业家`, example: `<AboutAuthor name="李明" avatar="https://avatars.githubusercontent.com/u/21008209?v=4" title="未来智联创始人兼 CEO" bio="连续创业者，致力于用科技改变生活。曾获年度创新人物奖，带领团队从零做到行业领先。" style="executive" avatarWidth="full" avatarShape="round" />` },
    ],
  },
]
...
function renderAboutAuthor(props: Record<string, string>): string {
  const { name, avatar, title, bio, style: styleProp, avatarWidth: avatarWidthProp, avatarShape: avatarShapeProp } = props
  const style = styleProp || `card`

  // 各风格的智能默认值：shape 和 size
  const styleDefaults: Record<string, { shape: string, size: string }> = {
    blue: { shape: `rect`, size: `large` },
    card: { shape: `rect`, size: `small` },
    badge: { shape: `round`, size: `small` },
    lawyer: { shape: `round`, size: `180` },
    executive: { shape: `round`, size: `large` },
  }
  const defaults = styleDefaults[style] || styleDefaults.card

  // 兼容旧属性 avatarSize
  const avatarSize = avatarWidthProp || props.avatarSize || defaults.size
  const avatarShape = avatarShapeProp || defaults.shape
  ...
  <section style="display: table; width: 100%; margin-bottom: 16px;">
    ${avatar ? `<section style="display: table-cell; vertical-align: middle; width: ${avatarW + 8}px;"><section style="width: ${avatarW}px; height: ${avatarW}px; border-radius: ${avatarR}; border: 2px solid rgba(255,255,255,0.4); background-image: url('${esc(avatar)}'); background-size: cover; background-position: center 20%;"></section></section>` : ``}
  ...
  ${bio ? `<p style="margin: 0; font-size: 14px; color: rgba(255,255,255,0.85); line-height: 1.8;">${esc(bio)}</p>` : ``}
</section>`
  }
  ...
  <section style="display: table; width: 100%; padding: 20px; border-bottom: 1px solid #f0f0f0;">
    ${avatar ? `<section style="display: table-cell; vertical-align: middle; width: ${avatarW + 8}px;"><section style="width: ${avatarW}px; height: ${avatarW}px; border-radius: ${avatarR}; background-image: url('${esc(avatar)}'); background-size: cover; background-position: center 20%;"></section></section>` : ``}
    <section style="display: table-cell; vertical-align: middle; padding-left: ${avatar ? '16' : '0'}px;">
  ...
  <section style="padding: 16px 20px;">
    ${title ? `<p style="margin: 0 0 8px; font-size: 14px; color: #666;">${esc(title)}</p>` : ``}
    ${bio ? `<p style="margin: 0; font-size: 14px; color: #999; line-height: 1.7;">${esc(bio)}</p>` : ``}
  </section>
</section>`
  }
  ...
  if (style === `lawyer`) {
    ...
    const avatarContainer = avatar
      ? `<section style="text-align: center; padding: 24px 20px 16px; background: ${lawyerBg};"><section style="display: inline-block; width: ${avatarW}px; height: ${avatarH}px; border-radius: ${avatarR}; background-image: url('${esc(avatar)}'); background-size: cover; background-position: center 20%;"></section></section>`
      : ``
