...
{{#each tags}}<span style="display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 13px; font-weight: 500; background: {{color}}1a; color: {{color}}; border: 1px solid {{color}}40;">{{item}}</span>{{/each}}
  ...
  },
  {
    ...
    examples: [
      ...
      { label: `律师`, example: `<AboutAuthor name="陈恒" avatar="https://raw.githubusercontent.com/BluerAngala/cc-md-editor/main/apps/web/public/images/chenheng.png" title="广东岭南律师事务所专职律师 / 产业投资专业委员会委员" bio="法学专业，爱好科技，略懂 AI 和自动化，业余折腾了一些法律科技有关的项目。如有志同道合的朋友，欢迎交流探讨。" style="lawyer" avatarSize="full" />` },
...
function renderAboutAuthor(props: Record<string, string>): string {
  ...
  <section style="display: table; width: 100%; margin-bottom: 16px;">
    ${avatar ? `<section style="display: table-cell; vertical-align: middle; width: ${avatarW + 8}px;"><img src="${esc(avatar)}" alt="${esc(name)}" style="width: ${avatarW}px; height: ${avatarW}px; object-fit: cover; border-radius: ${avatarR}; border: 2px solid rgba(255,255,255,0.4);" /></section>` : ``}
  ...
  <section style="display: table; width: 100%; padding: 20px; border-bottom: 1px solid #f0f0f0;">
    ${avatar ? `<section style="display: table-cell; vertical-align: middle; width: ${avatarW + 8}px;"><img src="${esc(avatar)}" alt="${esc(name)}" style="width: ${avatarW}px; height: ${avatarW}px; object-fit: cover; border-radius: ${avatarR};" /></section>` : ``}
    <section style="display: table-cell; vertical-align: middle; padding-left: ${avatar ? '16' : '0'}px;">
  ...
  ${avatar ? `<section style="text-align: center; padding: 24px 20px 16px; background: #f9f6f0;"><img src="${esc(avatar)}" alt="${esc(name)}" style="width: ${avatarW}px; height: auto; max-width: 100%; border-radius: ${avatarR}; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" /></section>` : ``}
  ...
  ${avatar ? `<section style="text-align: center; padding: 28px 20px 16px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);"><img src="${esc(avatar)}" alt="${esc(name)}" style="width: ${avatarW}px; height: auto; max-width: 100%; border-radius: ${avatarR}; box-shadow: 0 4px 16px rgba(0,0,0,0.3);" /></section>` : ``}
