...
- **白板画布**：基于 Excalidraw 的无限画布，支持手绘风格的流程图、思维导图、便签等
- **AI 生成想法**：输入自然语言描述，AI 自动生成 Mermaid 流程图并插入画布
  - 支持 7 种图表类型：流程图、时序图、类图、状态图、ER 图、甘特图、饼图
  - 流程图/状态图支持竖向（TD）和横向（LR）两种布局方向
  - Mermaid 模式：AI 生成 Mermaid 代码，SVG 预览后插入画布（可编辑元素）
  - JSON 模式（测试）：AI 直接生成 Excalidraw 元素 JSON，绕过 Mermaid 转换
- **优化想法**：将粗糙的语音转文字、零散笔记等输入整理为结构化图表大纲，再生成图表
- **法律专用模板**：内置诉讼流程图、法律关系图、案件时间轴、合同审查流程、利害关系人、仲裁流程图等提示词模板
- **历史记录**：自动保存生成历史，支持恢复提示词、快速插入画布、删除
- **AI 配置**：对话框内直接配置 API Key、模型、端点
- **便签墙**：支持创建、编辑、拖拽、缩放、分组、搜索便签
- **面板宽度记忆**：左右面板拖拽调整宽度后自动保存，下次加载恢复
## 功能特性
