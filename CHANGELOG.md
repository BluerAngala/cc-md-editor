...
- **想法库（白板 + AI 生成图表）**：新增基于 Excalidraw 的白板画布，支持 AI 从自然语言描述生成流程图、时序图、类图等 7 种图表类型，一键插入画布。内置法律专用提示词模板，支持历史记录管理和 AI 配置。
- **AI 生成想法对话框**：支持图表类型选择（流程图/时序图/类图/状态图/ER图/甘特图/饼图）、布局方向切换（竖向/横向）、优化想法（整理粗糙输入为结构化大纲）、Mermaid SVG 预览、历史记录查询/恢复/删除。
- **想法库面板宽度记忆**：左右面板拖拽调整宽度后自动保存，下次加载恢复。
